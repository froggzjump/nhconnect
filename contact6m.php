...
$query_contacts = "SELECT * FROM follows_contacts WHERE contact_wait <> 1 AND contact_date < unix_timestamp(now() - interval 180 day) AND contact_date > unix_timestamp(now() - interval 366 day) $sorder";
...
if ($row_userinfo['user_level'] != 1) {
mysql_select_db($database_follows, $follows);
$query_contacts = "SELECT * FROM follows_contacts WHERE contact_user = $whois AND contact_wait <> 1 AND contact_date < unix_timestamp(now() - interval 180 day) AND contact_date > unix_timestamp(now() - interval 366 day) $sorder" ;
$follows = mysql_query($query_contacts, $follows) or die(mysql_error());
$row_contacts = mysql_fetch_assoc($follows);
$totalRows_contacts = mysql_num_rows($follows);
...
<?php include('includes/header.php'); ?>
  ...
  <div class="leftcolumn">
    <h2><?php echo MINISTRY . " Follow-Ups (" . $totalRows_contacts . ")";?></h2> for <?php echo $row_userinfo['user_name']; ?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
		...
		<tr>
          ...
          <th><a href="?<?php echo $countdays; ?>">Contact Date</a></th>
...
$start_ts = time();
$end_ts = $row_contacts['contact_date'];
$numdays = dateDiffTs($start_ts, $end_ts);
$contacted = gmdate("m-d-y", $end_ts);
	  //$time_A = strtotime($row_contacts['contact_date']);
...
?>	
		...
		<tr <?php if ($row_count%2) { ?>bgcolor="#F4F4F4"<?php } ?>>
          ...
          <td > <?php if ($row_userinfo['user_level'] == 1) { ?><a href="delete.php?contact=<?php echo $row_contacts['contact_id']; ?>" class="button"><span class="del">Delete</span></a><?php } ?></td>
        </tr>
...
?>
<?php
//onclick="javascript:return "confirm('Are you sure you want to delete the FollowUp for: <?php echo $row_contacts['contact_first']; ?> 
